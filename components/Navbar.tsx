...
export const Navbar = () => {
    ...
    <div className="z-50 fixed flex justify-center w-full text-white font-bold">
        ...
        justify-center p-2 max-w-[400px] mx-auto">
            <ul className="flex flex-row p-2 space-x-8">
                {
                    navLinks.map((link,index)=>(
                        <li key={index}>
                            <Link href={link.path} className="transform hover:text-white/50 transition-all duration-300 
                                                            ease-in-out">
                                        {link.title}
                            </Link>
                        </li>
        ...
        <div onClick={toggleNav} className="md:hidden absolute top-5 right-14 border rounded z-50 
